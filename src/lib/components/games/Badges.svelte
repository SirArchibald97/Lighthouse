...
<div>
    ...
    <div class="flex flex-row gap-x-2">
        <p class="self-center text-gray-600">Toggle estimates</p>
        <button type="button" on:click={toggleEstimates} class="self-center group relative inline-flex h-5 w-10 flex-shrink-0 cursor-pointer items-center justify-center rounded-full" role="switch" aria-checked="false">
            ...
            <span class={`${estimates ? "bg-red-500" : "bg-gray-200"} pointer-events-none absolute mx-auto h-4 w-9 rounded-full transition-colors duration-200 ease-in-out`}></span>
            <span class={`${estimates ? "translate-x-5" : "translate-x-0"} pointer-events-none absolute left-0 inline-block h-5 w-5 translate-x-0 transform rounded-full border border-gray-200 bg-white shadow ring-0 transition-transform duration-200 ease-in-out`}></span>
        </button>
    </div>
    {/if}
</div>