...
<head>
    <title>{`${data.player.username}'s Stats` || "Unknown Player"}</title>
    <link rel="icon" href={`https://crafatar.com/avatars/${data.uuid}`} />
</head>
<main class="mx-24 my-4">
    ...
    {:else}
        ...
        <div class="flex flex-row gap-4">
            <!-- player card -->
            <div class="border-2 border-slate-600 bg-slate-300 rounded-lg p-4">
                <!-- username & rank -->
                <div class="flex flex-row">
                    <img src={`https://cdn.islandstats.xyz/ranks/${getRankIcon(data.player.ranks)}`} class="w-10 h-10 rounded-md" alt="" />
            ...
            <div class="flex flex-col shrink">
                <!-- info tabs -->
                <div class="flex flex-row text-xl mb-4 bg-slate-300 border-2 border-slate-600 rounded-md shrink">
                    <div class="flex flex-col">
                        <button class={`px-6 py-2 ${infoTab === "stats" ? "font-bold" : ""}`} on:click={() => switchInfoTab("stats")}>Stats</button>
                        {#if infoTab === "stats"}<span class="h-2 w-full bg-red-400 rounded-md"></span>{/if}
                    </div>
                    <div class="flex flex-col">
                        <button class={`px-6 py-2 ${infoTab === "party" ? "font-bold" : ""}`} on:click={() => switchInfoTab("party")}>Party</button>
                        {#if infoTab === "party"}<span class="h-2 w-full bg-red-400 rounded-md"></span>{/if}
                    </div>
                    <div class="flex flex-col">
                        <button class={`px-6 py-2 ${infoTab === "friends" ? "font-bold" : ""}`} on:click={() => switchInfoTab("friends")}>Friends</button>
                        {#if infoTab === "friends"}<span class="h-2 w-full bg-red-400 rounded-md"></span>{/if}
                    </div>
                </div>
                ...
                {:else if infoTab === "friends"}
                    <!-- friends -->
                    <div class="border-2 border-slate-600 bg-slate-300 rounded-lg p-4">
                        <p class="text-3xl font-bold mb-2">Friends ({friendsOnline} / {data.player.friends.length} online)</p>
                        <div class="flex flex-col gap-y-1">
                            {#each fullList[friendIndex] as friend}
                                <div class="flex flex-row gap-y-1">
                                    <img class="w-8 h-8 mr-2 rounded-md" src={`https://cdn.islandstats.xyz/ranks/${getRankIcon(friend.ranks)}`} alt={`${friend.username}'s Rank'`} />
                                    <a class="hover:underline" href={`/player/${friend.username}`}>
                                        <p class="text-lg">{friend.username}</p>
                                    </a>
                                    <span class={`ml-2 self-center w-4 h-4 rounded-full ${data.player.friends.find(f => f.username === friend.username).status.online ? "bg-green-500" : "bg-slate-300"}`}></span>
                                </div>
                            {/each}
                        </div>
                        <div class="flex flex-row gap-2 mt-4">
                            {#each fullList as list, count}
                                {#if count === friendIndex}
                                    <button class="bg-slate-500 text-white rounded-lg px-2 py-1" disabled>{count + 1}</button>
                                {:else}
