        ...
        <div class="flex flex-col mb-8 gap-y-8">
            <div class="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-8 mx-4 sm:mx-24 md:mx-4 lg:mx-32">
                {#each data.game.boards as board}
                    <Leaderboard name={board.name} stat={board.stat} display={board.display || null} ratio={board.ratio || null} icon={board.icon || null} title={board.tooltip} players={data.players} />
                {/each}
            </div>
        </div>
    {:else}
