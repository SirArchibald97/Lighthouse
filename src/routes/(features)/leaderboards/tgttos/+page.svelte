...
<div class="flex flex-col mb-8 gap-y-8">
    <div class="flex flex-row justify-between mt-6 mx-4 sm:mx-44">
        ...
        <div class="flex flex-row gap-x-2 self-center flex-grow justify-center">
            <img src="https://cdn.islandstats.xyz/games/tgttos/icon.png" alt="TGTTOS Logo" class="w-12 h-12 self-center" />
            <p class="text-center text-3xl font-semibold self-center">TGTTOS Leaderboards</p>
        </div>
    </div>
    ...
    <div class="flex flex-col mb-8 gap-y-8">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-8 mx-4 sm:mx-24 lg:mx-32">
            ...
            {#each badges.tgttos as badge}
                <Leaderboard name={badge.name} stat={`statistics.tgttos.badges.${badge.stat}`} players={data.players} badge={badge} game="tgttos" />
            {/each}
        </div>
    </div>
</div>
